...
const getOperatorForTimeRange = (duration) => {
...
const ArchiveViewer = () => {
  ...
  const [currentOperator, setCurrentOperator] = createSignal(null);
  const [data, setData] = createSignal([]);
  const [loading, setLoading] = createSignal(false);
  const [error, setError] = createSignal(null);
  const [debugLogs, setDebugLogs] = createSignal([]);
  const [showDebugData, setShowDebugData] = createSignal(false);
  ...
  const fetchData = async (retryCount = 0) => {
    try {
      ...
      addDebugLog(err.message, 'error', { error: err });
  ...
  const handleTimeRangeChange = (start, end, operator) => {
  ...
  createEffect(() => {
    let interval;
    if (autoRefresh()) {
      interval = setInterval(handleRefresh, AUTO_REFRESH_INTERVAL);
